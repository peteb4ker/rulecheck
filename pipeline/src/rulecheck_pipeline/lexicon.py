...
def validate(terms: list[dict]) -> list[str]:
    ...
    for entry in terms:
        ...
        if entry.get("category") not in CATEGORIES:
            errors.append(f"{term}: category must be one of {sorted(CATEGORIES)}")
...
def extract(texts: list[str], min_count: int = 1) -> list[dict]:
    ...
    for text in texts:
        for word in tokenize(text):
            ...
            forms[key][lower] = forms[key].get(lower, 0) + 1
